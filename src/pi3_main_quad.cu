...
//Total state dim is the total number of states we need to keep track of for lwpr updating
#define TOTAL_STATE_DIM 15
...
//Smoothing constants for velocity and acceleration updates, alphas are for velocity and betas for acceleration
#define ALPHA_X .6
#define BETA_X .6
...
private:
  ...
  ros::Subscriber sub;
public:
  float s[TOTAL_STATE_DIM];
  //state = [x, y, z, roll, pitch, yaw, x_dot, y_dot, z_dot, roll_dot, pitch_dot, yaw_dot, x_ddot, y_ddot, z_ddot] 
...
void StateUpdater::stateCallback(const pi3_crazyflie_pkg::quadrotor_state::ConstPtr& state_msg) {
  float dt = 1/(1.0*HZ);
  //Cartesian velocity, acceleration, and position updates
  float old_x_d = s[6];
  float old_y_d = s[7];
  float old_z_d = s[8];

  s[6] = ALPHA_X*s[6] + (1-ALPHA_X)*state_msg->x_dot;
  s[7] = ALPHA_Y*s[7] + (1-ALPHA_Y)*state_msg->y_dot;
  s[8] = ALPHA_Z*s[8] + (1-ALPHA_Z)*state_msg->z_dot;

  s[12] = BETA_X*s[12] + (1-BETA_X)*(s[6] - old_x_d)/dt;
  s[13] = BETA_Y*s[13] + (1-BETA_Y)*(s[7] - old_y_d)/dt;
  s[14] = BETA_Z*s[14] + (1-BETA_Z)*(s[8] - old_z_d)/dt;
  ...
  s[9] = ALPHA_ROLL*s[9] + (1-ALPHA_ROLL)*(state_msg->roll - s[3])/dt;
  s[10] = ALPHA_PITCH*s[10] + (1-ALPHA_PITCH)*(state_msg->pitch - s[4])/dt;
  s[11] = ALPHA_YAW*s[11] + (1-ALPHA_YAW)*(state_msg->yaw - s[5])/dt;
...
__device__ void rf_predict(RF_Predict *rf, float* pred_helper, float* x, int index, int t) {
  ...
  float dist = 0;
  /*For non-diagonal distance metrics
  /*for (i = 0; i < N; i++) {
    float dot = 0;
    for (j = 0; j < N; j++) {
      dot += rf->D[j*N + i]*xc[j];
  ...
  */
  //For diagonal distance metrics
  for (i = 0; i < N; i++) {
    dist += xc[i]*xc[i]*rf->D[i*N + i];
  }
...
int main(int argc, char** argv) {
  ...
  int a4 = lwpr_read_xml(&model4, model_x, e4);
  int a5 = lwpr_read_xml(&model5, model_y, e5);
  int a6 = lwpr_read_xml(&model6, model_z, e6);

  ROS_INFO("LWPR READ (0 success, -3 fail): %d, %d, %d, %d, %d, %d \n", e1[0], e2[0], e3[0], e4[0], e5[0], e6[0]);
  ...
  while (ros::ok()) {
    ...
    //LWPR Update
    /*double lwpr_input[11] = { (double)ros_state.s[2], (double)ros_state.s[3], (double)ros_state.s[4], (double)ros_state.s[5], (double)ros_state.s[6],
			      (double)ros_state.s[7], (double)ros_state.s[8], (double)u[0], (double)u[1], (double)u[2], (double)u[3] };
    */
    ros::spinOnce();
    /*
    //LWPR targets
    //roll target
    double out1 = ros_state.s[9];
    //pitch target
    double out2 = ros_state.s[10];
    //yaw target
    double out3 = ros_state.s[11];
    //x_ddot target
    double out4 = ros_state.s[12];
    //y_ddot target
    double out5 = ros_state.s[13];
    //z_ddot target 
    double out6 = ros_state.s[14];
    ...
    lwpr_update(&model4, lwpr_input, &out4, NULL, NULL);
    lwpr_update(&model5, lwpr_input, &out5, NULL, NULL);
    lwpr_update(&model6, lwpr_input, &out6, NULL, NULL);
    */
